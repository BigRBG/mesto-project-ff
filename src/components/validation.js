...
function checkInputValidity(validationConfig, formSelector, inputSelector){
    if (inputSelector.validity.patternMismatch) {                                                // Валидное регулярное значение даст - False
  ...
  } else {                                                                                       // Иначе True и результатом уже будет пустая строка, то есть, либо ошибки не будет, либо некастамизированная
    inputSelector.setCustomValidity("");
  }
    if (!inputSelector.validity.valid){                                                          // Поле не(!) соотвуетствует по всем параметрам валидности, то использовать функуцию раскрытия ошибки
        showInputError(validationConfig, formSelector, inputSelector, inputSelector.validationMessage)
    }else{                                                                                       // В случае соответсвтия поля и валидности, скрыть ошибку.
...
function clearValidation(formSelector, validationConfig){
    const errorSpanMessage = Array.from(formSelector.querySelectorAll(validationConfig.inputSelector))
    const submitButtonSelector = formSelector.querySelector(validationConfig.submitButtonSelector)

    errorSpanMessage.forEach((elErrorSpan) => {
        hideInputError(validationConfig, formSelector, elErrorSpan);
        elErrorSpan.setCustomValidity("")
    })

    toggleButtonState(validationConfig, errorSpanMessage, submitButtonSelector);
  }
